...
const QuotationView = ({ quotation, businessName, businessDetails, onBack, onEdit, onConvert }: QuotationViewProps) => {
  ...
  const generatePDF = async () => {
    ...
    // Bottom margin so the footer stays on-page
    const footerMargin = 20;
    let y = 15;

    // Ensure content that would run past the bottom starts on a new page
    ...
    doc.setTextColor(30, 30, 30);
    items.forEach((item) => {
      // One row per item; break to a new page if it won't fit
      ensureSpace(8);
      doc.setFillColor(249, 250, 251);
